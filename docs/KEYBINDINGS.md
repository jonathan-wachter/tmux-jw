...
| `C-k o` | **Cockpit popup** (v3.3, 2026-07-09): Claude-active session tabs on top (`↑` reaches them, blue when focused; `←/→`/tap browse another session *without leaving yours*); each window is a flush-left `N) name ────` entry with its recap below, plus a `+ new window` row at the bottom; on the selected entry `←/→` walk the **control chips** (only text inside `❯ ❮` highlighted, accelerator letter underlined) — `open` · one chip per other Claude session (move there; `cc-parking` always offered) · **(n)ew session** (spin window into a fresh session) · **(m)ove** (teleport to an absolute slot, like `C-k .`) · **(r)ename** (inline-edit the name) · **(c)lose** (red, two-step confirm, graceful `/exit`); n/m/r/c are direct accelerators · `⏎` run/open · `.` **global cross-session search** (type to filter windows across all sessions, shown with `· session` suffix; stays until Esc) · `t` sort · `s` **new blank session** (`cc-mmdd`, sized to your client) · `?` **tmux key-binding help** (live filterable 2-col reference of the prefix bindings, `↑↓` scroll, Esc closes) · number to jump · `q`/`Esc` close | `tmux-claude-dashboard-open.sh`; dropdown under the bar; full-screen on iPhone; shadows stock o (next pane) |
